<script lang="ts">
	...
	import { menglongApi } from '$lib/services/playground';
...
<div class="space-y-6">
	<div>
		<Heading tag="h1" class="text-3xl font-bold">AI 模型测试 Playground</Heading>
		<P class="mt-2">交互式AI模型测试环境，支持参数调整和结果分析</P>
	</div>
	...
	{#if showApiKeyInput}
		<Card class="mx-auto max-w-2xl">
			<div class="mb-6 text-center">
				<Heading tag="h2" class="mb-2 text-xl font-semibold">连接到 MengLong API</Heading>
	...
	{:else}
		...
		<Alert
			color="green"
			class="border-green-200 bg-green-50 transition-all duration-300 hover:shadow-md dark:border-green-600 dark:bg-green-900/10"
		>
			<div class="flex items-center justify-between">
				<div class="flex items-center">
					<svg
						class="mr-3 h-5 w-5 animate-pulse text-green-500 dark:text-green-400"
						fill="currentColor"
						viewBox="0 0 20 20"
					>
						<path
		...
		<div class="grid gap-6 lg:grid-cols-3">
			...
			<div class="space-y-4 lg:col-span-1">
				<Card class="shadow-sm transition-shadow duration-200 hover:shadow-md">
					<Heading tag="h3" class="mb-4 text-lg font-semibold">模型配置</Heading>
					<div class="space-y-4">
						<div class="transition-all duration-200">
							<Label class="mb-2 flex items-center justify-between">
								<span class="text-gray-900 dark:text-gray-100">模型选择</span>
								<Badge class="bg-blue-600 px-2 py-1 text-xs text-white dark:bg-blue-500"
									>{$availableModels.length} 个可用</Badge
								>
							</Label>
							<Select
								value={$selectedModel}
								onchange={handleModelChange}
								class="transition-all duration-200 hover:border-blue-400 focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100 dark:hover:border-blue-500"
							>
						...
						<div
							class="-m-3 rounded-lg p-3 transition-all duration-200 hover:bg-gray-50 dark:hover:bg-gray-800/50"
						>
							<Label class="mb-3 flex items-center justify-between">
								...
								<Badge class="bg-blue-600 px-2 py-1 font-mono text-xs text-white dark:bg-blue-500"
									>{$parameters.temperature}</Badge
								>
							</Label>
							<Input
								type="range"
								...
								class="h-2 w-full cursor-pointer appearance-none rounded-lg bg-gray-200 transition-all duration-200 dark:bg-gray-700"
								oninput={(e) => {
							...
							/>
							<div
								class="mt-2 flex justify-between text-xs font-medium text-gray-600 dark:text-gray-300"
							>
								<span>精确 (0)</span>
								<span>平衡 (1)</span>
								<span>创造 (2)</span>
						...
						>
							...
							<Input
								...
								class="transition-all duration-200 hover:border-blue-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100 dark:hover:border-blue-500"
								oninput={(e) => {
									const target = e.target as HTMLInputElement;
				...
				<Card class="shadow-sm transition-shadow duration-200 hover:shadow-md">
					<Heading tag="h3" class="mb-4 flex items-center justify-between text-lg font-semibold">
						<span class="text-gray-900 dark:text-gray-100">输入内容</span>
						<span
							class="rounded bg-gray-100 px-2 py-1 text-xs font-normal text-gray-600 dark:bg-gray-700 dark:text-gray-300"
						>
							{$inputText.length} 字符
						</span>
					</Heading>
					<div class="relative">
						<Textarea
							...
							class="resize-none font-mono text-sm leading-relaxed transition-all duration-200 hover:border-blue-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-900 dark:hover:border-blue-500"
							value={$inputText}
							oninput={handleInputChange}
						/>
						{#if $inputText.length > 0}
							<button
								class="absolute top-2 right-2 rounded-md bg-gray-100 p-1.5 transition-colors duration-200 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600"
					...
					<div class="mt-4 space-y-3">
						<div
							class="flex items-center space-x-3 rounded-lg border border-gray-200 bg-gradient-to-r from-gray-50 to-gray-100 p-3 transition-all duration-200 hover:shadow-sm dark:border-gray-700 dark:from-gray-800 dark:to-gray-800/50"
						>
							<Toggle bind:checked={streamMode} class="focus:ring-2 focus:ring-blue-500" />
							<div class="flex-1">
								<Label class="cursor-pointer text-sm font-medium text-gray-900 dark:text-gray-100"
									>流式输出</Label
								>
							...
							<Badge
								class="px-2 py-1 text-xs {streamMode
						...
						<Button
							color="blue"
							class="w-full py-3 text-base font-medium transition-all duration-200 hover:scale-[1.02] hover:shadow-lg active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:scale-100 disabled:hover:shadow-none"
							disabled={!canSubmit}
						...
						>
							{#if isLoading}
								...
								处理中...
							{:else}
								<PaperPlaneOutline class="mr-2 h-5 w-5" />
								发送请求
							{/if}
						</Button>
						{#if $inputText.trim().length === 0}
							<div
								class="flex items-center justify-center space-x-2 rounded-lg bg-gray-50 p-2 text-xs text-gray-600 dark:bg-gray-800 dark:text-gray-300"
							>
			...
			<div class="space-y-4 lg:col-span-2">
				<Card class="shadow-sm transition-shadow duration-200 hover:shadow-md">
					...
					<div
						class="relative min-h-[300px] overflow-hidden rounded-lg border-2 border-gray-200 bg-gradient-to-br from-gray-50 to-gray-100 p-6 dark:border-gray-700 dark:from-gray-900 dark:to-gray-800"
					>
						{#if $currentOutput}
							<div
								class="font-sans leading-relaxed whitespace-pre-wrap text-gray-900 dark:text-white"
							>
						...
						{:else if hasError}
							<!-- 增强的错误展示 -->
							<div class="flex h-full min-h-[250px] flex-col items-center justify-center space-y-4">
								<div class="rounded-full bg-red-100 p-4 dark:bg-red-900/20">
									<ExclamationCircleOutline class="h-12 w-12 text-red-500 dark:text-red-400" />
								</div>
						...
						{:else}
							<!-- 空状态 -->
							<div
								class="flex h-full min-h-[250px] flex-col items-center justify-center text-gray-500 dark:text-gray-400"
							>
								<svg
								...
								<P class="text-center">输入内容并点击发送按钮开始测试</P>
							</div>
						{/if}
					</div>
				</Card>
				...
				<Card class="shadow-sm transition-shadow duration-200 hover:shadow-md">
					<Heading tag="h3" class="mb-4 text-lg font-semibold">请求详情</Heading>
					<div class="grid grid-cols-2 gap-3">
						<div
							class="rounded-lg border border-blue-200 bg-gradient-to-br from-blue-50 to-blue-100 p-4 transition-all duration-200 hover:shadow-md dark:border-blue-800 dark:from-blue-900/20 dark:to-blue-800/20"
						>
						...
						<div
							class="rounded-lg border border-purple-200 bg-gradient-to-br from-purple-50 to-purple-100 p-4 transition-all duration-200 hover:shadow-md dark:border-purple-800 dark:from-purple-900/20 dark:to-purple-800/20"
						>
							<div class="mb-2 flex items-center space-x-2">
								<svg
							...
							<div class="text-xl font-bold text-purple-900 dark:text-purple-100">
								{requestDetails.tokenUsage}
							</div>
						</div>
						<div
							class="rounded-lg border border-green-200 bg-gradient-to-br from-green-50 to-green-100 p-4 transition-all duration-200 hover:shadow-md dark:border-green-800 dark:from-green-900/20 dark:to-green-800/20"
						>
							<div class="mb-2 flex items-center space-x-2">
								<svg
							...
							<div
								...
								class:text-green-700={requestDetails.status === '成功'}
								class:dark:text-green-300={requestDetails.status === '成功'}
								class:text-red-700={requestDetails.status === '失败'}
								class:dark:text-red-300={requestDetails.status === '失败'}
								class:text-gray-700={requestDetails.status === '就绪'}
								class:dark:text-gray-300={requestDetails.status === '就绪'}
							>
								{requestDetails.status}
							</div>
						</div>
						<div
							class="rounded-lg border border-amber-200 bg-gradient-to-br from-amber-50 to-amber-100 p-4 transition-all duration-200 hover:shadow-md dark:border-amber-800 dark:from-amber-900/20 dark:to-amber-800/20"
						>
							<div class="mb-2 flex items-center space-x-2">
								<svg
							...
							<div
								class="truncate text-sm font-semibold text-amber-900 dark:text-amber-100"
								title={requestDetails.model}
							>
