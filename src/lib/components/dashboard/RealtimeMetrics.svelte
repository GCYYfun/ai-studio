		...
		return [
			{
				...
				trend: stats.requestsPerMinute > 100 ? 'high' : stats.requestsPerMinute > 50 ? 'medium' : 'low',
				color: stats.requestsPerMinute > 100 ? 'text-red-600' : stats.requestsPerMinute > 50 ? 'text-yellow-600' : 'text-green-600'
			},
			{
				label: '平均响应时间',
				value: formatDuration(stats.averageResponseTime),
				trend: stats.averageResponseTime > 1000 ? 'high' : stats.averageResponseTime > 500 ? 'medium' : 'low',
				color: stats.averageResponseTime > 1000 ? 'text-red-600' : stats.averageResponseTime > 500 ? 'text-yellow-600' : 'text-green-600'
			},
			{
				label: '错误率',
				value: formatPercentage(stats.errorRate / 100),
				trend: stats.errorRate > 5 ? 'high' : stats.errorRate > 1 ? 'medium' : 'low',
				color: stats.errorRate > 5 ? 'text-red-600' : stats.errorRate > 1 ? 'text-yellow-600' : 'text-green-600'
			},
			{
				label: '活跃连接',
				value: formatNumber(stats.activeConnections),
				trend: stats.activeConnections > 1000 ? 'high' : stats.activeConnections > 500 ? 'medium' : 'low',
				color: stats.activeConnections > 1000 ? 'text-red-600' : stats.activeConnections > 500 ? 'text-yellow-600' : 'text-green-600'
			}
		];
	});
...
<Card class="relative">
	<div class="flex items-center justify-between mb-4">
		...
		<div class="flex items-center space-x-2">
			...
			{#if autoUpdate}
				<div class="w-2 h-2 bg-green-400 rounded-full animate-pulse" title="实时更新中"></div>
			{/if}
		</div>
	</div>
		...
		<div class="space-y-4">
			{#each Array(4) as _}
				<div class="flex justify-between items-center">
					<div class="h-4 bg-gray-300 dark:bg-gray-600 rounded animate-pulse w-1/3"></div>
				...
				<div class="flex justify-between items-center">
					...
					<div class="flex items-center space-x-2">
						...
						<div class="w-2 h-2 rounded-full {
							metric.trend === 'high' ? 'bg-red-400' :
							metric.trend === 'medium' ? 'bg-yellow-400' :
							'bg-green-400'
						}"></div>
					</div>
				</div>
